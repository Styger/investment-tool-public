...
def show_pbt_analysis():
    ...
    if st.button(get_text("start_analysis"), key="pbt_run"):
        if not ticker:
            st.error(get_text("invalid_input", "Please enter a ticker symbol"))
        else:
            with st.spinner(f"Calculating PBT for {ticker}..."):
                try:
                    ...
                    with col1:
                        st.metric(
                            get_text("pbt_price", "PBT Buy Price"), f"${price:,.2f}"
                        )
                        st.info("Based on 8-year payback period")
                    ...
                    with col2:
                        if table:
                            ...
                            df = df.rename(
                                columns={
                                    "Jahr": get_text("year", "Year"),
                                    "Einnahme": get_text("revenue", "Income"),
                                    "Summe_Cashflows": get_text(
                                        "cumulative", "Cumulative"
                                    ),
                                }
                            )
                            df[get_text("revenue", "Income")] = df[
                                get_text("revenue", "Income")
                            ].apply(lambda x: f"${x:,.2f}")
                            df[get_text("cumulative", "Cumulative")] = df[
                                get_text("cumulative", "Cumulative")
                            ].apply(lambda x: f"${x:,.2f}")
                    ...
                    st.error(
                        get_text("analysis_error", "PBT analysis failed: {}").format(
                            str(e)
                        )
                    )
